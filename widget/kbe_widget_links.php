  ...
  //=======> How to display the widget on the screen.
    function widget($args, $widgetData) {
        ...
        ?>
            ...
            ?>
            <a class="kbe_add_button" title="Neuer Beitrag" href="<?php echo get_site_url() ?>/wp-admin/post-new.php?post_type=kbe_knowledgebase"><i class="mdf mdf-lg mdf-plus"></i></a>
            <?php
	    }
            if (is_singular() and !is_page() and current_user_can( "edit_posts", $post_id ) ) {
            ?>
            <a class="kbe_edit_button" title="Beitrag Editieren" href="<?php echo get_site_url() ?>/wp-admin/post.php?action=edit&post=<?php echo $post_id ?>"><i class="mdf mdf-lg mdf-pencil"></i></a>
            <?php
            }

        }
        if (!is_singular() and !is_page()) {
            $cat_link = get_category_feed_link( $queried_object->term_id );
            ?>
