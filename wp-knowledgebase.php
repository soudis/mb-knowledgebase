<?php
/*
  Plugin Name: Mühlböck Knowledgebase
  Plugin URI: http://wordpress.org/plugins/mb-knowledgebase
  Description: Simple and flexible knowledgebase plugin for WordPress (forked from kb-knowledgebase)
  Author: Enigma Plugins / Florian Humer
  Version: 1.1.4
  Author URI: http://enigmaplugins.com / muehlboeck.com
...
//  Require Category Tree Widget file
require "widget/kbe_widget_category_tree.php";
//  Require Category Tree Widget file
require "widget/kbe_widget_links.php";
...
<div id="live-search">
    <div class="kbe_search_field">
        <form role="search" method="get" id="searchform" class="clearfix" action="<?php echo home_url( '/' ); ?>" autocomplete="off">
            <input type="text" onfocus="if (this.value == '<?php _e("Artikel durchsuchen...", "kbe") ?>' || this.value=='Artikel durchsuchen...') {this.value = '';}" onblur="if (this.value == '')  {this.value = '<?php _e("Artikel durchsuchen...", "kbe") ?>';}" value="<?php _e("Artikel durchsuchen...", "kbe") ?>" name="s" id="s" />
...
add_shortcode('kbesearchform', 'kbe_search_form');

//=========> KBE Search Form
function kbe_search_form_cat($category_id){
?>
<!-- #live-search -->
<div id="live-search">
    <div class="kbe_search_field">
        <form role="search" method="get" id="searchform" class="clearfix" action="<?php echo home_url( '/' ); ?>" autocomplete="off">
            <input type="text" onfocus="if (this.value == '<?php echo get_term_by('id', $category_id, KBE_POST_TAXONOMY)->name ?> durchsuchen...' || this.value=='Search Articles...') {this.value = '';}" onblur="if (this.value == '')  {this.value = '<?php _e("Search Articles...", "kbe") ?>';}" value="<?php echo get_term_by('id', $category_id, KBE_POST_TAXONOMY)->name ?> durchsuchen..." name="s" id="s" />
            <!--<ul id="kbe_search_dropdown"></ul>-->
            <input type="hidden" name="post_type" value="kbe_knowledgebase" />
            <input type="hidden" id="search_term_id" name="search_term_id" value="<?php echo $category_id; ?>" />
        </form>
    </div>
</div>
<!-- /#live-search -->
...
add_action('wp_footer', 'kbe_collapsible');
function kbe_collapsible(){ 
    ?>
...
function kbe_breadcrumbs(){
    ...
    function print_term($tax_term) {
        if ($tax_term->parent <> 0) {
            print_term(get_term_by('id', $tax_term->parent, KBE_POST_TAXONOMY));
        }
        ?>
                <li>
                    <a href="<?php echo get_term_link($tax_term->slug, KBE_POST_TAXONOMY) ?>">
...
<!--            <li><a href="<?php echo home_url(); ?>"><?php _e('Home','kbe'); ?></a></li> 
            ...
            <?php print_term(get_queried_object()); ?>
...
function search_term_id($query) {
    // Check if posts only value has been set and equal to 1. Return if not.
    //echo("test".$_GET['search_term_id']);
    if ( ! isset( $_GET['search_term_id'] )) {
        return false;
    }
    //echo "ishome:".$query->is_home()." mainquery:".$query->is_main_query()." admin:".is_admin();
    //echo $_GET['search_term_id'];
